                ...
                <div className="bg-white p-6 rounded-xl shadow-md">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="pt-4 mt-4 border-t">
                            ...
                            <p className="text-xs text-slate-500 mb-3">Standard values come from <em>standard yard balance.xlsx</em>. Available stock is auto-filled from <em>Yard Balance</em> data (upload via Data Manager) using the most recent date on or before the current date.</p>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <InputField label="Standard Stock – Centre" id="standardStockCentre" type="number" value={props.standardStockCentre} onChange={handleNumberChange(props.setStandardStockCentre)} readOnly={props.isReadOnly} tooltip="Target yard balance for all non-gate centers (from standard yard balance.xlsx)." />
                                <InputField label="Standard Stock – Gate" id="standardStockGate" type="number" value={props.standardStockGate} onChange={handleNumberChange(props.setStandardStockGate)} readOnly={props.isReadOnly} tooltip="Target yard balance at the gate (from standard yard balance.xlsx)." />
                                <InputField label="Available Stock – Centre" id="availableStockCentre" type="number" value={props.availableStockCentre} onChange={handleNumberChange(props.setAvailableStockCentre)} readOnly={props.isReadOnly} tooltip="Actual current yard balance across all centers. Auto-filled from uploaded Yard Balance data for the current date." />
                                <InputField label="Available Stock – Gate" id="availableStockGate" type="number" value={props.availableStockGate} onChange={handleNumberChange(props.setAvailableStockGate)} readOnly={props.isReadOnly} tooltip="Actual current yard balance at the gate. Auto-filled from uploaded Yard Balance data for the current date." />
                            </div>
                        </div>
                    </div>
                </div>
